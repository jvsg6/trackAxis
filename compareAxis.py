...
distance=150 #Расстояние при котором точки можно считать совпадающими
percent=80 #Процент совпадения точек, при котором можносчитать совпадающими оси следа
...
def main():
	...
	for j in range(len(lst)): #перебираем все функционалы(1ый функц. для сравнения)(что сравниваем)
		...
		for k in range(j, len(lst)): #перебира по всем функц. (2ой функционал для сравнения)(с чем сравниваем)
			...
			if float(n/length*100.0)>float(percent):
	...
	wb.save(mypath + "/Compare_{0}_{1}.xlsx".format(str(distance), str(percent)))
